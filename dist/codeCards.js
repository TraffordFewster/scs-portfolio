...
for (var data in projects) {
  var key = data;
  data = projects[data];
  htmlStr += "<div class='projectCard codeCard'  onclick='projectPopup(" + key + ")'>" + "<div class='imgHolder'>" + "<img src='" + data.img + "' alt='" + data.title + " preview image'>" + '</div>' + "<div class='textHolder'>" + '<h1>' + data.title + '</h1>' + "<h4>View Project <i class='fas fa-arrow-right'></i></h4>" + '</div>' + '</div>'; // Thanks IE for not support multiline strings so fun :)
}
